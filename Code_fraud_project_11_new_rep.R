...
colnames(readability)<-c("id","Flesch")
...
final_data<-cbind(all_id[,-3], ngram_data[,-1], ns_ngram_data, readability[,-1], my_fcms, LIWC, endata[,3], dependencies_data)

wfd<-reshape(final_data, idvar = "id", timevar = "section", direction = "wide")
fraud_mark<-reshape(all_id, idvar = "id", timevar = "section", direction = "wide")
fraud_mark[is.na(fraud_mark[,3]),3]<-fraud_mark[is.na(fraud_mark[,3]),2]
final<-cbind(as.factor(fraud_mark[,3]), wfd)
colnames(final)[1]<-"fraud"

final<-cbind(final,0)
final<-na.omit(final)
final<-final[,-(which(colSums(final[,-1])==0)+1)]

source("Get_membership.R")

memb_id<-rbind(cbind(memberdata[,1],memberdata[,2]),cbind(memberdata[,1]*1000,memberdata[,2]))
groups<-rep(0,length(final[,1]))
for(i in 1:length(final[,2])){
  if(length(memb_id[which(memb_id[,1]%in%final[i,2]==TRUE),2])>0){
    groups[i]<-memb_id[which(memb_id[,1]%in%final[i,2]==TRUE),2]
  }
}
...
while(a<.6 | a>.65 | b<.495 | b>.505){
  selects<-sample(unique(groups), sample(1:(length(unique(groups))/2)))
  inTraining<-which(groups%in%selects)
  a<-length(final[inTraining,2])/length(final[,2])
  b<-sum(as.numeric(final[inTraining,1])-1)/length(final[inTraining,1])
}


training <- final[ inTraining,-2]
testing  <- final[-inTraining,-2]