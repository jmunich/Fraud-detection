...
parsesa<-spacy_parse(tot_corpus_clean[1:299], dependency=TRUE)
parsesb<-spacy_parse(tot_corpus_clean[300:600], dependency=TRUE)
parsesc<-spacy_parse(tot_corpus_clean[601:ndoc], dependency=TRUE)

parses<-rbind(parsesa, parsesb, parsesc)
...
file_gn<-file_gn[,varchoice]
file_fn<-file_fn[,varchoice]
file_gn_jlsp<-file_gn_jlsp[,varchoice]
file_fn_jlsp<-file_fn_jlsp[,varchoice]

file_gn_new<-aggregate(file_gn, by=list(gen_id[,1]), FUN=mean)                 
file_fn_new<-aggregate(file_fn, by=list(fraud_id[,1]), FUN=mean)                 
                 
LIWC<-rbind(file_gn_new[,-1], file_gn_jlsp, file_fn_new[,-1], file_fn_jlsp)

### Combine

final_list<-list(as.matrix(all_id[,2],ncol=1), ngram_data[,-1], ns_ngram_data, as.matrix(readability[,-1],ncol=1), my_fcms, LIWC, as.matrix(endata[,3],ncol=1), dependencies_data)
